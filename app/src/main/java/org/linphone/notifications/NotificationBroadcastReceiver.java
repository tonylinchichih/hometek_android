    ...
    public void onReceive(final Context context, Intent intent) {
        ...
        if (intent.getAction().equals(Compatibility.INTENT_REPLY_NOTIF_ACTION)
            ...
            if (intent.getAction().equals(Compatibility.INTENT_ANSWER_CALL_NOTIF_ACTION)) {
                // terminate other calls
                for (int i = 0; i < core.getCalls().length; i++) {
                    Log.e(
                            "getRemoteAddress="
                                    + core.getCalls()[i].getRemoteAddress().asStringUriOnly()
                                    + ", state="
                                    + core.getCalls()[i].getState());
                    if (!remoteAddr.equals(
                            core.getCalls()[i].getRemoteAddress().asStringUriOnly())) {
                        core.getCalls()[i].terminate();
                    }
                }
